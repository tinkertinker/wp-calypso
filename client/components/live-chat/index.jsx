...
const chatIsAvailable = propExists( 'available' )
...
const renderTimelineItem = ( { item, isCurrentUser } ) => {
	let [ message, meta ] = item
	return (
		<div className={ classnames( 'live-chat-timeline-message', { userMessage: isCurrentUser } ) } key={ meta.id }>
			<div className="message-text">{ renderMessage( { isCurrentUser, message, nick: meta.nick } ) }</div>
...
const welcomeMessage = () => <div className="live-chat-welcome">This is the beginning of your chat history with WordPress.com support. A chat history will be stored here.</div>
